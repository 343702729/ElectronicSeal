...
//        builder.addPart(Headers.of("Authorization", authorization), body);
     ...
     * @param picLocalPath
     */
    public void uploadUserHeadDo(String authorization, final RxAppCompatActivity context, String picLocalPath, final BaseInfoUpdate infoUpdate){
        if(TextUtils.isEmpty(picLocalPath))
            return;
//        String temporaryPath = PictureUtil.getTemporaryPic(picLocalPath);
        final File file = context.getFileStreamPath(picLocalPath);
//        final File file = new File(picLocalPath);
        if(file==null||!file.exists())
            return;
        RequestBody body=RequestBody.create(MediaType.parse("multipart/form-data"),file);//表单类型
